...
- **Auth:** protected endpoints require `Authorization: Bearer <access_token>`
  (tokens are minted by the eID / Google / SSO login flows below)
...
- **Body cap:** `/auth/*`, `/sso/*` and `/provider/*` bodies are limited to 4
...
plus **Google OAuth** account-linking and **dgov SSO** (see the SSO section).
...
The platform as an **OIDC consumer** of dgov SSO (a second login flow alongside
eID). Pre-login, so no bearer required; body-capped (4 KiB) and run under a
service RLS identity (the callback upserts into `users`).

| Method | Path | Description |
|--------|------|-------------|
| POST | `/sso/start` | Begin the SSO authorization flow (returns the redirect URL). |
| POST | `/sso/callback` | Complete the web flow (code exchange → token pair). |
| POST | `/sso/native` | Native/mobile SSO exchange. |
