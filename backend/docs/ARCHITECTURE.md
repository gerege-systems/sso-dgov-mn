...
`internal/http/routes/route_auth.go`, `route_eidprofile.go`, and `route_sso.go`.
...
**3. dgov SSO (OIDC consumer)** (`pkg/oidc`, `SSO_*`): a second login option that
delegates to `sso.dgov.mn`, plus a native/PKCE mobile flow (`SSO_NATIVE_CLIENT_ID`).
...
> registers only the eID / Google / SSO / refresh / logout endpoints above.
...
`org`, `gov`, `security`, `userintegrations`, `ssouser`) — the `withRLS(ctx, fn)`
...
| Auth (identity)   | eID RP + Google OAuth + dgov SSO        | `usecases/auth`, `pkg/{eid,google,oidc}`   |
