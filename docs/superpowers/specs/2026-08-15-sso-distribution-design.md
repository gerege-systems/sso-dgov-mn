...
## 12. Хэрэгжилтийн үеийн хазайлт

Барих явцад дизайнаас зөрсөн зүйлс, шалтгаантайгаа:

1. **Цөмийг `v1.6.0`-д бэхэлсэн**, v1.5.0 биш — spec бичих үед v1.6.0 tag аль
   хэдийн push хийгдсэн байв.

2. **Нэг түлхүүр, `SSO_SECRET_KEY`** — federation-д тусдаа
   `SSO_FEDERATION_KEY` гэж төлөвлөсөн боловч SCIM токен ч мөн адил
   шифрлэгдэх тул хоёр нэрээр нэг зүйлийг хийх шалтгаан байсангүй.
   `internal/secret` дундын багц болов.

3. **`accessreview` гурван эрхтэй, хоёр биш** — `read` / `decide` / `manage`.
   Кампанит ажлыг нээж хаадаг хүн, түүн дотор шийдвэр гаргадаг хүн хоёр өөр
   байх ёстой; платформын read/manage дүрэм үүнийг илэрхийлж чадахгүй тул
   `RoutePermissionPrefix()` хоосон буцааж, бүлэг бүр өөрийн эрхээ нэрлэдэг
   болов.

4. **Sessions нь `devices` хүснэгтийг холбоогүй** — цөмийн схемд сесс ба
   төхөөрөмжийн хооронд холбоос байхгүй байв. Төхөөрөмжийн тухай мэдэх зүйл нь
   `user_agent` ба `ip_address` хоёр.

5. **Хуулбарласан цөмийн апп таван болов** — `organisation`, `egov`,
   `documents`, `sso-clients`, `reports`. `contacts` ба `esign` нь цөмийн
   `00058`/`00059` миграцаар `documents`, `organisation` руу шингэсэн байв.

6. **Гурав дахь upstream PR олдов** — SCIM дараалалд ажил нэмэх цорын ганц зам
   нь `resync`. Цөмд хэрэглэгчийн амьдралын мөчлөгийн event байхгүй.

7. **`deploy.yml` илгээгээгүй** — шилжүүлэх шийдвэр гараагүй байхад host,
   secret нь үүсээгүй pipeline бичих нь ажиллах үедээ л буруу зүйл хийж чадах
   pipeline бичих явдал. Compose болон Dockerfile нь бэлэн, домэйн шилжих
   өдөр нэмнэ.

8. **Нэг бодит алдаа тестээр баригдав** — `sessions.live` тогтмол `s.` alias
   агуулж байсан бөгөөд UPDATE-үүд alias-гүй тул ажиллах үедээ л унана.
   Компайлер харах боломжгүй; DB-тэй тест барив.
